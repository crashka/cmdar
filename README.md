...
* **virtualenv** (or `python3-venv` package on Ubuntu)
    * This is not a strict requirement, but the `dar_server.sh` shell script wrapper
      ...
      the server](#start-the-server) below)
* **VLC 3.0**
...
### Clone this project and create virtual environment ###
...
### Install package dependencies in virtual environment ###
...
environment.  See [Configuration](#configuration), below, for more information on the content/format
for the config file.
...
The currently defined "section" names within a config "profile" are as follows:
